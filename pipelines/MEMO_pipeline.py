...
class MEMORndDenoisePipeline(MEMOEdgePipeline):
    @torch.inference_mode()
    def __call__(
        self,
        images: List[Union[np.array, torch.Tensor]],
        ...
        for i in self.progress_bar(range(num_inference_steps)):
            ...
            for j in range(confidence.shape[0]):
                # _, select_index = torch.topk(confidence[j], k=transfer_tokens[j, i].item(), largest=True)
                valid_index = torch.where(confidence[j] != -torch.inf)[0]
                rnd_index = torch.randperm(len(valid_index))
                valid_index = valid_index[rnd_index]
                select_index = valid_index[:transfer_tokens[j, i]]
                transfer_index[j, select_index] = True
...
class MEMOEdgeAutoTransferTokenPipeline(MEMOEdgePipeline):
    @torch.inference_mode()
    def __call__(
        self,
        images: List[Union[np.array, torch.Tensor]],
        ...
        guidance_scale: float = 4.0,
        denoise_conf_thres: float = 0.98, # if prediction confidence is larger than this threshold, we transfer the token, otherwise, keep it as masked
    ) -> Union[ImagePipelineOutput, Tuple]:
        ...
        for i in self.progress_bar(range(max_inference_steps)):
            ...
            mask_index = (masked_edges == mask_token_id) # (batch_size, h, w)
            if mask_index.sum() == 0:
                # if no masked tokens left, we can stop the denoising process
                break
            ...
            for j in range(confidence.shape[0]):
                ...
                if i < max_inference_steps - 1:
                    num_transfer = max((confidence[j] > denoise_conf_thres).sum().item(), int(np.ceil(0.3 * this_masked_tokens))) # at least 1% of the masked tokens should be transferred
...
class MEMOEdgeLocalMaximumPipelineWithTimer(MEMOEdgePipeline):
    @torch.inference_mode()
    def __call__(
        self,
        images: List[Union[np.array, torch.Tensor]],
        ...
        for i in self.progress_bar(range(max_inference_steps)):
            t0 = time()
            mask_index = (masked_edges == mask_token_id)
            num_mask = mask_index.sum(dim=[1, 2])
            if num_mask < 1:
                break
            mask_ratio = num_mask / (image_h * image_w)  # (B, )
            ...
            for j in range(confidence.shape[0]):
                ...
                # ).squeeze(-1) # being both masked and local maxima
                # rnd_idx = torch.randperm(unmasked_index.shape[0])
                select_index = (masked_edges[j] == mask_token_id) & local_max_map & (confidence[j] > conf_thres)
                ...
                if i < max_inference_steps - 1:
                    ...
                    # select_index = torch.nonzero(masked_edges[j] == mask_token_id).squeeze(-1)  # transfer all masked tokens
                    select_index = (masked_edges[j] == mask_token_id)  # transfer all masked tokens
                transfer_index[j, select_index] = True

            # masked_edges[transfer_index] = x_0[transfer_index]
            masked_edges = torch.where(transfer_index, x_0, masked_edges)
            masked_edges = rearrange(masked_edges, 'b (h w) -> b h w', h=image_h, w=image_w)
            # save the probabilities of the transferred tokens
            # pred_probs[transfer_index] = p[transfer_index]
            pred_probs = torch.where(transfer_index.unsqueeze(-1), p, pred_probs)
            t1 = time()
            total_time += t1-t0
