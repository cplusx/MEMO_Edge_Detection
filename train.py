...
from misc_utils.train_utils import get_models, get_edge_trainer, get_logger, get_callbacks, get_dataset
...
if __name__ == '__main__':
    ...
    edge_model_configs = args.edge_trainer
    edge_trainer_model = get_edge_trainer(
        models,
        edge_model_configs=edge_model_configs,
    )
    ...
    if args['resume']:
        ...
        if os.path.exists(ckpt_path):
            ...
            if args['manual_load']:
                ...
                if 'module' in ckpt:
                    ckpt = ckpt['module']
                ema_model_weights = {k.replace('ema_denoiser.module.', ''): v for k, v in ckpt.items() if 'ema_denoiser.module.' in k}
                edge_trainer_model.denoiser.load_state_dict(ema_model_weights)
        ...
        ckpt_path = None
    trainer.fit(
        edge_trainer_model, train_loader, val_loader,
        ckpt_path=ckpt_path
    )